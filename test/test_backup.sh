...
singleFile="$TEST_PREFIX-singlefile"
...
prepare_test()
{
	...
	dd if=/dev/random bs=1500 count=5 iflag=fullblock > $singleFile 2> /dev/null
...
verify()
{
	...
	diff $singleFile $BACKUP_DIR$singleFile
	if [[ $? -ne 0 ]]; then
		echo "TEST FAILED" >&2
		return 1
	fi
